      ...
      <TableBody>
        {holdings.map((h) => (
          <TableRow key={h.symbol}>
            ...
            <TableCell className="text-right">{formatCurrency(h.avgCost)}</TableCell>
            <TableCell className="text-right">{formatCurrency(h.currentPrice)}</TableCell>
            <TableCell className="text-right font-semibold">{formatCurrency(h.currentValue)}</TableCell>
            <TableCell className={cn(
              "text-right font-medium",
              h.unrealizedGain >= 0 ? "text-green-500" : "text-destructive"
            )}>
              {formatCurrency(h.unrealizedGain)}
              <div className="text-xs text-muted-foreground">{formatPercent(h.unrealizedGainPercent)}</div>
            </TableCell>
          </TableRow>
        ))}
      </TableBody>
